...
#define SMBUS_SCK_H()        HAL_GPIO_WritePin(Tem_SCL_GPIO_Port,Tem_SDA_Pin,GPIO_PIN_SET)
#define SMBUS_SCK_L()        HAL_GPIO_WritePin(Tem_SCL_GPIO_Port,Tem_SDA_Pin,GPIO_PIN_RESET)
#define SMBUS_SDA_H()        HAL_GPIO_WritePin(Tem_SDA_GPIO_Port,Tem_SCL_Pin,GPIO_PIN_SET)
#define SMBUS_SDA_L()        HAL_GPIO_WritePin(Tem_SDA_GPIO_Port,Tem_SCL_Pin,GPIO_PIN_RESET)
#define SMBUS_SDA_PIN()	     HAL_GPIO_ReadPin(Tem_SDA_GPIO_Port,Tem_SCL_Pin) 
...
void Temperature_measure(void){	//高温度警告
    if(temp>=35&&Temp_Broadcast==0){
		ESP8266_Send_ESP32("11",100);
        Voice_Broadcast(0x18);
		data_T=1;
